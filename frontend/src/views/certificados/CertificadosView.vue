  ...
  <div class="p-6 md:p-10 space-y-8 animate-in fade-in duration-500">
    ...
    <div v-if="currentView === 'lista'" class="space-y-8">
      <div class="flex items-end justify-between border-b border-slate-200 pb-6">
          <div>
              <h2 class="text-3xl md:text-4xl font-black text-umsa-blue tracking-tighter uppercase italic">Emisión de Certificados</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-4">
          <div @click="openWorkspace('Especialidad en Biofertilizantes')" class="course-card bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm cursor-pointer group flex flex-col hover:-translate-y-1 hover:border-slate-400 hover:shadow-[0_15px_30px_rgba(1,34,72,0.08)] transition-all">
              <div class="flex justify-between items-center mb-6">
                  <span class="bg-umsa-blue text-white text-[9px] font-black px-3 py-1 rounded-full uppercase">Diplomado</span>
              ...
              <h3 class="text-xl font-black text-umsa-blue leading-tight mb-2 uppercase">Especialidad en Biofertilizantes</h3>
              <p class="text-[10px] font-bold text-slate-400 uppercase mb-8">Aprobados: 35 Alumnos | 4 Ponentes</p>
              <div class="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between text-[10px] font-black text-umsa-gold uppercase">
    ...
    <div v-else-if="currentView === 'workspace'" class="space-y-8 animate-in slide-in-from-right-8 duration-500">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-4 space-y-6">
                <div class="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100">
                    <h3 class="text-xs font-black text-umsa-blue uppercase tracking-widest mb-6 border-b border-slate-100 pb-4">Etiquetas Dinámicas</h3>
                    ...
                    <div v-if="audienceType === 'estudiantes'" class="space-y-3">
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 group cursor-copy hover:border-primary-dark transition-colors">
                            <span class="text-[10px] font-black text-umsa-blue" v-pre>{{ESTUDIANTE_NOMBRE}}</span>
                            <span class="material-symbols-outlined text-slate-300 group-hover:text-umsa-blue text-sm">content_copy</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 group cursor-copy hover:border-primary-dark transition-colors">
                            <span class="text-[10px] font-black text-umsa-blue" v-pre>{{NOTA_FINAL}}</span>
                            <span class="material-symbols-outlined text-slate-300 group-hover:text-umsa-blue text-sm">content_copy</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 group cursor-copy hover:border-primary-dark transition-colors">
                            <span class="text-[10px] font-black text-umsa-blue" v-pre>{{CARGA_HORARIA}}</span>
                            <span class="material-symbols-outlined text-slate-300 group-hover:text-umsa-blue text-sm">content_copy</span>
                        </div>
                    </div>

                    <div v-else class="space-y-3">
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 group cursor-copy hover:border-slate-400 transition-colors">
                            <span class="text-[10px] font-black text-umsa-blue" v-pre>{{PONENTE_NOMBRE}}</span>
                            <span class="material-symbols-outlined text-slate-300 group-hover:text-umsa-gold text-sm">content_copy</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 group cursor-copy hover:border-slate-400 transition-colors">
                            <span class="text-[10px] font-black text-umsa-blue" v-pre>{{TEMA_IMPARTIDO}}</span>
                            <span class="material-symbols-outlined text-slate-300 group-hover:text-umsa-gold text-sm">content_copy</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 group cursor-copy hover:border-slate-400 transition-colors">
                            <span class="text-[10px] font-black text-umsa-blue" v-pre>{{HORAS_DICTADAS}}</span>
                            <span class="material-symbols-outlined text-slate-300 group-hover:text-umsa-gold text-sm">content_copy</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-200 dark:border-gray-800">
                        <div v-if="statusGeneracion === 'generando'" class="space-y-4">
                            ...
                            <p class="text-xs text-center font-bold text-slate-500 uppercase tracking-widest animate-pulse">
                                Procesando PDFs... {{progress}}%
                            </p>
                        </div>
                        
                        <div v-else-if="statusGeneracion === 'completado'" class="space-y-3">
                            <button @click="sendToAll" class="w-full py-3.5 bg-green-600 text-white font-bold text-xs uppercase rounded-xl shadow-sm hover:bg-green-700 transition-all flex items-center justify-center gap-2 tracking-widest border border-green-700">
                                <span class="material-symbols-outlined text-[16px]">send</span>
                                Enviar por Correo
                            </button>
                            <p class="text-[10px] text-center font-bold text-slate-500 uppercase tracking-widest mt-2">
                                <span class="text-green-600 font-black">✓</span> {{ audienceType === 'estudiantes' ? '35' : '4' }} PDFs listos
                            </p>
                        </div>

                        <button v-else @click="generarCertificados" class="w-full py-3.5 bg-umsa-blue text-white font-bold text-xs uppercase rounded-xl shadow-md shadow-umsa-blue/20 hover:bg-primary-accent transition-all flex items-center justify-center gap-2 tracking-widest border border-blue-600">
                            <span class="material-symbols-outlined text-[16px]">play_circle</span>
                            Iniciar Generación ({{ audienceType === 'estudiantes' ? '35' : '4' }})
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-xs font-black text-umsa-blue uppercase tracking-widest">Cola de Emisión: {{ audienceType === 'estudiantes' ? 'Estudiantes' : 'Ponentes' }}</h3>
            </div>
              ...
              <table class="w-full text-left whitespace-nowrap">
                  ...
                  <tbody class="divide-y divide-slate-100 text-xs">
                      <tr class="hover:bg-slate-50 transition-colors">
                          <td class="px-6 md:px-8 py-4 font-bold text-umsa-blue uppercase">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-umsa-blue/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
        <div class="bg-white rounded-[2rem] w-full max-w-lg p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-2xl font-black text-umsa-blue uppercase tracking-tighter italic flex items-center gap-2">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input v-model="googleSlidesUrl" type="url" placeholder="Ej: https://docs.google.com/presentation/d/.../edit" class="w-full border-umsa-blue border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-umsa-gold focus:ring-0 transition-colors bg-slate-50 text-umsa-blue font-bold">
                </div>
            </div>
            
            <div class="mt-8 flex gap-3">
                <button @click="isModalOpen = false" class="flex-1 px-4 py-3 border-umsa-blue border-slate-200 text-slate-500 font-black text-[10px] uppercase rounded-xl hover:bg-slate-50 transition-colors tracking-widest">Cancelar</button>
                <button @click="linkSlides" :disabled="!googleSlidesUrl" :class="googleSlidesUrl ? 'bg-umsa-blue hover:bg-primary-accent text-white shadow-lg' : 'bg-slate-200 text-slate-400 cursor-not-allowed'" class="flex-[2] px-4 py-3 font-black text-[10px] uppercase rounded-xl transition-all tracking-widest">
